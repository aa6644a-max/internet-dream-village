...
**최초 작성:** 2026-05-25  
**최종 업데이트:** 2026-06-04 (3차)  
**상태:** demo.html 온보딩 UX + 일일 여정 시스템 구체화 완료 → **다음: Phase 1 실제 개발 착수** (이슈 #6)

---

## 현재 상태

PRD 1~4 설계 완료. demo.html 5-Stage 시연 프로토타입 전체 완성. 이슈 #9 종료.

### 2026-06-04 3차 세션 완료 내역 — 온보딩 UX + 일일 여정 시스템

**/grill-me 그릴링으로 확정된 설계:**

| 결정 항목 | 내용 |
|---|---|
| 온보딩 플로우 | 4단계: welcome → signup → profile → **카드뉴스 튜토리얼 (신규)** |
| 홈 UX | 자유 메뉴 선택 → **오늘의 여정 Step 카드 (선형 잠금)** |
| 여정 순서 | 마음일기 → 미래의 나 → 자가진단 → 🎁 네모로직 PDF |
| 리셋 주기 | 매일 (dailyProgress 상태) |
| 완료 조건 | 마음일기=AI분석 완료, 미래의 나=이미지 생성 완료, 자가진단=결과 도달 |
| 보상 | 3단계 완료 시 오늘의 네모로직 PDF 언락 (날짜 기반 퍼즐 번호) |

**demo.html 구현 내역:**
- `ob-3` 추가: 카드뉴스 8장 슬라이드 (이전/다음 네비게이션, 도트, 건너뛰기, 마지막=시작하기)
- 홈 `home-grid` → `journey-section` + `step-cards` 교체
- `unlockStep(step)` / `updateStepCards()` — 잠금 상태 DOM 동기화
- `tutGo(n)` — 슬라이드 전환 + 마지막에 completeProfile 바인딩
- `dailyProgress = { diary, ciref, diag }` 상태 관리
- `downloadNonogram()` — 날짜 기반 퍼즐 번호 alert (실제 PDF 연동 준비됨)
- 완료 트리거: `runDiaryAnalysis` / `genFutureImage` / `showDiagResult` 에 각각 unlockStep 삽입

### 2026-06-04 2차 세션 완료 내역

| Stage | 내용 | 상태 |
|---|---|---|
| 2 | 미로 워크시트(`screen-maze`) + 사용량 기록(`screen-usage`) | ✅ |
| 3 | (Stage 2에 통합 완료) | ✅ |
| 4 | 관리자 뷰 — 학생 현황 대시보드·위기신호·학생 상세 | ✅ |
| 5 | 모바일 폰프레임 토글 (393×852 아이폰 프레임, 다이나믹 아일랜드, 바텀탭) | ✅ |

**demo.html 현재 기능 (전체):**
- 온보딩 4단계 (welcome→signup→profile→**카드뉴스 튜토리얼**)
- 홈: **오늘의 여정 Step 카드** (마음일기→미래의 나→자가진단→네모로직 순서 잠금)
- 학생 앱: 마음일기·미래의 나(CIREF)·자가진단·미로 워크시트·사용량 기록
- 관리자 뷰: 학생 현황(KPI 4개+테이블)·위기신호(필터+확인처리)·학생 상세(자가진단 바+사용량 바+일기+상담메모)
- 📱 폰프레임 버튼: 393×852 아이폰 CSS 프레임, 상태바(실시간시계), 바텀탭 5종, 외부 종료 버튼
- 관리자 모드 토글: `body.admin-mode` CSS 클래스, 사이드바 nav 분기
...
| `demo.html` | 완전 시연 프로토타입 — **Stage 1~5 전체 완료** (온보딩·학생앱·관리자뷰·폰프레임) |
...
- **이슈 #9:** demo.html 완전 프로토타입 Stage 2~5 ✅ **완료**
- **이슈 #10:** cardnews.html 개선 — 탭 TYPE화·해시태그 색연동·HTML 공유 (미완)
...
**Phase 1 개발 착수** (이슈 #6) — demo.html 프로토타입 + 온보딩 UX 완성됨. 이제 실제 Next.js 앱 개발.

> **네모로직(nonogram) 구현 예정** — Phase 3 게임 보상. demo.html에 `downloadNonogram()` 자리 확보됨. 실제 구현 시 날짜 시드 기반 퍼즐 생성 + jsPDF 연동.

### demo.html 완전 프로토타입 진행 현황 ✅ 전체 완료

| Stage | 내용 | 상태 |
|---|---|---|
| 1 | Shell + 온보딩 (welcome → signup → profile) | ✅ |
| 2 | 미로 워크시트(`screen-maze`) + 사용량 기록(`screen-usage`) | ✅ |
| 3 | (Stage 2에 통합) | ✅ |
| 4 | 관리자 뷰 — 학생 현황·위기신호·학생 상세 | ✅ |
| 5 | 모바일 폰프레임 토글 (393×852, 다이나믹 아일랜드, 바텀탭) | ✅ |

### Phase 1 개발 즉시 실행 순서 (이슈 #6)

1. Supabase 프로젝트 생성 (리전: ap-northeast-1 서울, 프로젝트명: internet-dream-village)
...
8. 수퍼바이저 계정 자동 생성 (환경변수 기반)
9. Vercel 연동 + 첫 배포

### 발표 관련 참고
- `demo.html` — Chrome 전체화면으로 시연. 📱 폰프레임 버튼으로 모바일 시연 가능
- `발표대본_print.html` — Chrome Ctrl+P → PDF 저장 (배경 그래픽 체크)
- `인터넷드림마을_발표.pptx` — Pretendard 폰트 설치 필요 (미설치 시 맑은 고딕)
- `cardnews.html` — "📤 HTML 파일로 공유" 버튼으로 단일 파일 내보내기
